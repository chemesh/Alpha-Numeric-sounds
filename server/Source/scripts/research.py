...
LEVAD_BAMIDBAR = "C:\\Users\\roy12\\PycharmProjects\\AlphaNumericSounds\\resources\\לבד במדבר  -  גיא מזיג.wav"
BILL = "C:\\Users\\roy12\\PycharmProjects\\AlphaNumericSounds\\resources\\Grover Washington Jr. feat. Bill Withers - Just The Two of Us [HQ].wav"
...
def run_timed_func(func, *kargs, **kwargs):
    print(f"running function {func.__name__}")
    print(f"parameters {kargs}, {kwargs}")
    res = None
    start = time.time()
    res = func(*kargs, **kwargs)
    end = time.time()
...
def main():
    urls = ['https://www.youtube.com/watch?v=5llD4epFi6s', 'https://www.youtube.com/watch?v=PJ0u5c9EF1E']
    songs_paths = [LEVAD_BAMIDBAR, BILL]
    if not os.path.exists(LEVAD_BAMIDBAR):
        print("songs doesnt exists on this machine. downloading from YT")
        yt_manager = YTManager()
        songs_paths = yt_manager.download(urls)
        lb = "\n"
        print(f"finished downloading songs: {lb.join(songs_paths)}")

    songs = [Song.from_wav_file(path, duration=60) for path in songs_paths]
    print("finished extracting songs")

    engine = EA_Engine(Logger())
    print("create ea engine instance. starting mixing")
    res = run_timed_func(
        engine.mix,
        *songs,
        max_gens=10,
    ...
    sf.write("C:\\Users\\roy12\\PycharmProjects\\AlphaNumericSounds\\output\\out.wav", res.data, res.sr)
